namespace Ant
{
    ...
    public class GloveMeshesController : MonoBehaviour
    {
        public bool IsGloveRendered { get; private set; }
        ...
        {
            get => m_HandMeshRenderers;
            set
            {
                m_HandMeshRenderers = value;
        ...
        private void Awake()
        {
            m_HandMeshRenderers = GetComponentsInChildren<Renderer>(true);
        }
        ...
        void UpdateRendererVisibility()
        {
            ...
            {
                //m_HandMeshRenderers.enabled = true;
                IsGloveRendered = true;
                ShowGloveMeshes(true);
            }
            else if (!m_HandIsTracked && m_HideMeshWhenTrackingIsLost)
            {
                //m_HandMeshRenderers.enabled = false;
                IsGloveRendered = false;
                ShowGloveMeshes(false);
            }
        }

        private void ShowGloveMeshes(bool _flag)
        {
            var tmp_length = m_HandMeshRenderers.Length;
            for (int i = 0; i < tmp_length; i++)
            {
                var tmp_mesh = m_HandMeshRenderers[i];
                if (tmp_mesh == null) continue;
                tmp_mesh.enabled = _flag;
            }
        }
    }
}